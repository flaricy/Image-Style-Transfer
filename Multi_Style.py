...
def get_styles(image_shape_, device_):
    ...
    """
    style_Xs_ = [preprocess(img, image_shape_).to(device_) for img in Style_Imgs]
    styles_Ys_ = [extract_features(x, Content_Layers, Style_Layers)[1] for x in style_Xs_]
    return style_Xs_, styles_Ys_
...
def compute_loss(X, content_Y_hat, style_Y_hat, content_Y_, style_Y_grams, lap_Y_, lap_Y_hat):
    """
    Calculate the content, style, and total variance losses respectively
    :param X: 当前生成的图片
    :param content_Y_hat: 内容层的预测
    :param style_Y_hat: 风格层的预测
    :param content_Y_: 内容层的参考值
    :param style_Y_grams: 风格层gram的参考值
    :param lap_Y_: 拉普拉斯的参考值
    :param lap_Y_hat: 拉普拉斯的预测值
    :return: (内容loss, 风格loss, 方差loss, 拉普拉斯loss, 总loss)
    """
    content_l = [content_loss(Y_hat, Y) * content_weight for Y_hat, Y in zip(
        content_Y_hat, content_Y_)]
    # style_l = [[style_loss(Y_hat, Y) * w * style_weight for Y_hat, Y in zip(
    #     style_Y_hat, gram)] for gram, w in zip(style_Y_grams, Style_Interpolation)]
    style_l = 0
    for gram, w in zip(style_Y_grams, Style_Interpolation):
        cur_l = [style_loss(Y_hat, Y) * style_weight for Y_hat, Y in zip(style_Y_hat, gram)]
        style_l += sum(cur_l) / len(cur_l) * w
    tv_l = tv_loss(X) * tv_weight
    lap_l = lap_loss(lap_Y_, lap_Y_hat) * lap_weight
    # Add up all the losses
    l_ = style_l + sum(content_l) + sum([tv_l]) + sum([lap_l])
    return content_l, style_l, tv_l, lap_l, l_
...
def get_inits(X, device_, lr, style_Ys_):
    """
    initialize gen_img to our content image (i.e. X)
    :param X: 初始的图像
    :param device_: gpu/cpu
    :param lr: 学习率
    :param style_Ys_: 风格层的参考值
    ...
    styles_Y_grams = [[gram(Y) for Y in style_Y_] for style_Y_ in style_Ys_]
    return gen_img(), styles_Y_grams, trainer


# ---- training ----
def train(X, contents_Y_, styles_Ys_, device_, lr, num_epochs, lr_decay_epoch):
    """
    训练
    :param X: 初始图像
    :param contents_Y_: 图像层参考值
    :param styles_Ys_: 风格层参考值
    :param device_: gpu/cpu
    :param lr: 学习率
    ...
    X, styles_Y_grams, trainer = get_inits(X, device_, lr, styles_Ys_)
        ...
        if isinstance(trainer, torch.optim.LBFGS):  # 训练器采用L-BFGS
            def closure():
                ...
                _, _, _, _, ll = compute_loss(
                    X, content_Y_hat, style_Y_hat, contents_Y_, styles_Y_grams, Lap_Y, lap_Y_hat)
                ll.backward()
                loss_ = ll.item()
                return ll
            ...
            lap_Y_hat = lap_conv(X)
            _, _, _, _, ll = compute_loss(
                X, contents_Y_hat, styles_Y_hat, contents_Y_, styles_Y_grams, Lap_Y, lap_Y_hat)
            loss = ll.item()
            print(f"Loss = {loss}")
...
Style_Imgs = [d2l.Image.open(p + n + '.jpeg') for p, n in zip(Style_Paths, Style_Names)]

Content_X, Content_Y, Lap_Y = get_contents(IMAGE_SHAPE, DEVICE)
_, Style_Ys = get_styles(IMAGE_SHAPE, DEVICE)

Output = train(Content_X, Content_Y, Style_Ys, DEVICE, 0.8, 200, 25)
...
Output_Img.save(f'./results/{Content_Name + "-" + Style_Name}_multistyle_9_1.jpeg', 'JPEG')
